...
Aug_9_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:16) %>% 
  mutate(Site = "LP") %>%
  mutate(Location = c(rep("LWR", 3), rep("MID", 3), rep("UPR", 1), 
                      rep("LWR", 3), rep("MID", 3), rep("UPR", 3))) %>%
  mutate(Design = c(rep("BDA", 7), rep("REF", 9))) %>%
...
# DIC .csv
write.csv(Aug_9_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_9_DIC.csv")
...
# SGF .csv
Aug_9_SGF <- as_tibble(Final_gas_df) %>% 
  slice(17:52) %>% 
  mutate(Site = "LP") %>%
...
write.csv(Aug_9_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_9_SGF.csv")
