...
Aug_30_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-08-30","2021-08-29")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(Aug_30_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_30_DIC.csv")
...
# SGF .csv
Aug_30_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-08-30","2021-08-29")) %>%
...
write.csv(Aug_30_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_30_SGF.csv")
