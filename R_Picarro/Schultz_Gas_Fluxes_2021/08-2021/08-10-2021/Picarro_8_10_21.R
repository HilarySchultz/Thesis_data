...
Aug_10_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(Site = "FH") %>%
...
# DIC .csv
write.csv(Aug_10_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_10_DIC.csv")
...
# SGF .csv
Aug_10_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "FH") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(Aug_10_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_10_SGF.csv")
