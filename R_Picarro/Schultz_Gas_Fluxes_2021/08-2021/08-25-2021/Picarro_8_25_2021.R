...
filtered_data <- if_else(rawdata$X12CO2_dry>300, rawdata$X12CO2_dry, 0.0)
...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak != 56 & peak != 49) # deselecting for unwanted peaks
...
# peak CO2 19 was terrible, so needed to make sure that function worked for bad and normal peaks
# this is also a test to see if I need to adjust the window_size hyperparameter
bad_peak <- carbondioxide_df %>% filter(peak == 19) %>%
  arrange(INDEX) # bad peak
bad_test_result <- CO2_clean_peaks(bad_peak) # call clean peaks function on bad peak


x = bad_peak$EPOCH_TIME
y = bad_peak$X12CO2_dry
plot(x, y, col = ifelse(y < 632 & y > 619,'purple', 'black')) # eyeballed plateau in purple
lines(bad_test_result$EPOCH_TIME, bad_test_result$X12CO2_dry, col = "green") # overlay to check


normal_peak <- carbondioxide_df %>% filter(peak == 1) %>%
  arrange(INDEX)# normal peak
normal_test_result <- CO2_clean_peaks(normal_peak) # call clean peaks function on normal peak

plot(normal_peak$EPOCH_TIME, normal_peak$X12CO2_dry)
lines(normal_test_result$EPOCH_TIME, normal_test_result$X12CO2_dry, col = "green")
...
Aug_25_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(Site = "LP") %>%
...
# DIC .csv
write.csv(Aug_25_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_25_DIC.csv")
...
# SGF .csv
Aug_25_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "LP") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(Aug_25_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_25_SGF.csv")
...
# THIS FILE HAS BEEN RUN 11/18/21 