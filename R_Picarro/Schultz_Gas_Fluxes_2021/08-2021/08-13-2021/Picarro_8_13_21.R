...
Aug_13_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-08-13","2021-08-12")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(Aug_13_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_13_DIC.csv")
...
# SGF .csv
Aug_13_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-08-13","2021-08-12")) %>%
...
write.csv(Aug_13_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_13_SGF.csv")
