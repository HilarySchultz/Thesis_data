...
Aug_26_DIC <- as_tibble(Final_gas_df) %>% 
  slice(37:54) %>% 
  mutate(Site = "FH") %>%
...
# DIC .csv
write.csv(Aug_26_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Aug_26_DIC.csv")
...
# SGF .csv
Aug_26_SGF <- as_tibble(Final_gas_df) %>% 
  slice(1:36) %>% 
  mutate(Site = "FH") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(Aug_26_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Aug_26_SGF.csv")
