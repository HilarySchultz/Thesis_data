...
Sept_11_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-09-11","2021-09-10")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(Sept_11_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Sept_11_DIC.csv")
...
# SGF .csv
Sept_11_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-09-11","2021-09-10")) %>%
...
write.csv(Sept_11_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Sept_11_SGF.csv")
