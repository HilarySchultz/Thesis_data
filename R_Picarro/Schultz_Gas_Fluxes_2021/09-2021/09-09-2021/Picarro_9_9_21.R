...
filtered_data <- if_else(rawdata$X12CO2_dry>500, rawdata$X12CO2_dry, 0.0)
...
LP_DIC <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(91:108)) %>%
  mutate(Site = "LP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-09-09","2021-09-08")) 


FH_DIC <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(73:90)) %>%
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-09-09","2021-09-07")) 

Sept_9_DIC <- bind_rows(LP_DIC, FH_DIC) %>%
...
# DIC .csv
write.csv(Sept_9_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/Sept_9_DIC.csv")
...
FH_SGF <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(1:36)) %>%
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-09-09","2021-09-07"))


Sept_9_SGF <- bind_rows(LP_SGF, FH_SGF) %>%
  mutate(chamber = rep(chamber_vector, 2)) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(Sept_9_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/Sept_9_SGF.csv")
