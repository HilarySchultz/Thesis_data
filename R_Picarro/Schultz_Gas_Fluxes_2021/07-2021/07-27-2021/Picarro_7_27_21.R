...
July_27_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(Site = "FH") %>%
...
# DIC .csv
write.csv(July_27_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/July_27_DIC.csv")
...
# SGF .csv
July_27_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "FH") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(July_27_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/July_27_SGF.csv")
