...
July_26_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(Site = "LP") %>%
...
# DIC .csv
write.csv(July_26_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/July_26_DIC.csv")
...
# SGF .csv
July_26_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "LP") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(July_26_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/July_26_SGF.csv")
