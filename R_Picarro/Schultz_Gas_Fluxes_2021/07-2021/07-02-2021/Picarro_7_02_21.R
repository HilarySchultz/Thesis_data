...
July_2_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-07-02","2021-07-01")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(July_2_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/July_2_DIC.csv")
...
# SGF .csv
July_2_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-07-02","2021-07-01")) %>%
...
write.csv(July_2_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/July_2_SGF.csv")
