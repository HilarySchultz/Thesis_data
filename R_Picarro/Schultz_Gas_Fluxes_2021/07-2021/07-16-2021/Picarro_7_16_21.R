...
July_16_DIC <- as_tibble(Final_gas_df) %>% 
  slice(1:18) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-07-16","2021-07-15")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(July_16_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/July_16_DIC.csv")
...
# tmp_row <- Final_gas_df[49,]
# Final_gas_df[49,] <- Final_gas_df[50,]
# Final_gas_df[50,] <- tmp_row

# SGF .csv
July_16_SGF <- as_tibble(Final_gas_df) %>% 
  slice(19:54) %>% 
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-07-16","2021-07-15")) %>%
...
write.csv(July_16_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/July_16_SGF.csv")
