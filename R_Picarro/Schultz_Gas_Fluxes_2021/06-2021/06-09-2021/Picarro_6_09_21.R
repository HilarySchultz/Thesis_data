...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(5:27, 29:41, 45:80))# deselecting for unwanted peaks

# filtered_rawdata %>% filter(peak == 29) %>% ggplot(aes(INDEX, X12CO2_dry)) + geom_point()
...
LP_SGF <- as_tibble(Final_gas_df) %>% 
  slice(1:36) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-06-09","2021-06-07")) %>%
  mutate(Site = "LP") %>%
...
FH_SGF <- as_tibble(Final_gas_df) %>% 
  slice(37:72) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-06-09","2021-06-08")) %>%
  mutate(Site = "FH") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF"))

June_9_SGF <- bind_rows(LP_SGF, FH_SGF)

write.csv(June_9_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/June_9_SGF.csv")
...
# THIS FILE HAS BEEN RUN 11/22/21 