...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(15:32,42:59)) # deselecting for unwanted peaks
...
FH_DIC <- as_tibble(Final_gas_df) %>%
  slice(1:18) %>%
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-04","2021-06-02"))

TP_DIC <- as_tibble(Final_gas_df) %>%
  slice(1:18) %>%
  mutate(Site = "TP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-04","2021-06-03"))

June_04_DIC <- bind_rows(FH_DIC, TP_DIC) %>%
...
# DIC .csv
write.csv(June_04_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/June_04_DIC.csv")
