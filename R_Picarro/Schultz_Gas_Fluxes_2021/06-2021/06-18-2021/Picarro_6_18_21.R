...
June_18_DIC <- as_tibble(Final_gas_df) %>% 
  slice(10:27) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-06-18","2021-06-17")) %>%
  mutate(Site = "TP") %>%
...
# DIC .csv
write.csv(June_18_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/June_18_DIC.csv")
...
# SGF .csv
June_18_SGF <- as_tibble(Final_gas_df) %>% 
  slice(28:63) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-06-18","2021-06-17")) %>%
  mutate(Site = "TP") %>%
...
write.csv(June_18_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/June_18_SGF.csv")
