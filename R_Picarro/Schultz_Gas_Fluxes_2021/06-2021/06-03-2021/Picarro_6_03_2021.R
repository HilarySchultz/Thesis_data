...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(30:49)) # deselecting for unwanted peaks
...
June_03_DIC <- as_tibble(Final_gas_df) %>% 
  mutate(Site = "LP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-03","2021-06-01")) %>%
...
# DIC .csv
write.csv(June_03_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/June_03_DIC.csv")

# NO SOIL GAS FLUXES!!
# THIS FILE HAS BEEN RAN 11/18/21
