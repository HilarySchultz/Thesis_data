...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(7:33, 36:41, 51:68, 78:95, 96:107, 109:132))
...
LP_DIC <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(51:68)) %>% 
  mutate(Site = "LP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-16","2021-06-14")) 
  
FH_DIC <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(78:95)) %>% 
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-16","2021-06-15"))

June_16_DIC <- bind_rows(LP_DIC, FH_DIC) %>% 
...
# DIC .csv
write.csv(June_16_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/June_16_DIC.csv")
...
LP_SGF <- as_tibble(Final_gas_df) %>% 
  filter(peak %in% c(7:33, 36:41)) %>% 
  mutate(Site = "LP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-16","2021-06-14")) %>%
  mutate(chamber = c(rep.int(1,3), rep.int(2,3), rep.int(3,3), rep.int(4,3), rep.int(5,3),
                     rep.int(6,3), rep.int(7,3), rep.int(8,3), rep.int(9,3),
                     rep.int(11,3), rep.int(12,3)))

FH_SGF <- as_tibble(Final_gas_df) %>% 
  filter(peak %in% c(96:107, 109:132)) %>% 
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-16","2021-06-15")) %>%
  mutate(chamber = chamber_vector)


June_16_SGF <- bind_rows(LP_SGF, FH_SGF) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

# SGF .csv
write.csv(June_16_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/June_16_SGF.csv")
