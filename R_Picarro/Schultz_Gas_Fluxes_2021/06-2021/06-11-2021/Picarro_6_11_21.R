...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(5:13, 15:41)) # deselecting for unwanted peaks
...
# SGF .csv
June_11_SGF <- as_tibble(Final_gas_df) %>% 
  mutate(DATE = replace(DATE, DATE == "2021-06-11", "2021-06-10")) %>%
  mutate(Site = "TP") %>%
  mutate(chamber = chamber_vector) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(June_11_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/June_11_SGF.csv")
...
plot(rawdata$EPOCH_TIME, rawdata$X12CO2_dry, type = "l")
points(carbondioxide_cleaned_df$EPOCH_TIME, carbondioxide_cleaned_df$X12CO2_dry, col = "green")


# NO DIC SAMPLES ON THIS DAY! 
