...
filtered_rawdata <- rawdata %>% 
  ...
  filter(peak %in% c(1:36, 37:72, 73:93, 95:109))
...
LP_DIC <- as_tibble(Final_gas_df) %>%
  ...
  mutate(Site = "LP") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-30","2021-06-28")) 


FH_DIC <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(19:36)) %>%
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-30","2021-06-29")) 

June_30_DIC <- bind_rows(LP_DIC, FH_DIC) %>%
...
# DIC .csv
write.csv(June_30_DIC, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_DIC_2021/June_30_DIC.csv")
...
FH_SGF <- as_tibble(Final_gas_df) %>%
  filter(peak %in% c(73:93, 95:109)) %>%
  mutate(Site = "FH") %>%
  mutate(DATE = replace(DATE, DATE == "2021-06-30","2021-06-29"))


June_30_SGF <- bind_rows(LP_SGF, FH_SGF) %>%
  mutate(chamber = rep(chamber_vector, 2)) %>%
  mutate(Design = if_else(chamber <= 6, "BDA", "REF")) 

write.csv(June_30_SGF, 
          file = "/Users/hilaryschultz/Desktop/Thesis/R_Picarro/Picarro_SGF_2021/June_30_SGF.csv")
